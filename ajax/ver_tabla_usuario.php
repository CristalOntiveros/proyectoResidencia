...
<?php
$query_usuario= "SELECT * FROM jefes  ORDER BY nombre ";
    ...
    <tbody>
      <?php do {?> 
        <tr>
          ...
          <td align="center">
              <?php    if($row_usuario['Rol_idRol']==1)  { ?> 
                <form  method="get" action="javascript:admi('<?php echo $row_usuario['id_jefes']; ?>')">
                  <button type="submit" class="btn btn-primary btn-xs">Administrador</button>
                </form>
              <?php  }   else {?> 

                <form  method="get" action="javascript:usuario('<?php echo $row_usuario['id_jefes']; ?>')"> 
                  <button type="submit" class="btn btn-info btn-xs">Usuario</button>
                </form>
              <?php  } ?> 
          </td> 
          <!-- quite de aquí el codigo Estado-->
          
            <td>
               <a class="btn btn-primary btn-sm fun"  data-toggle="modal" data-target="#foto<?php echo $row_usuario['id_jefes']; ?>">
                <i class='glyphicon glyphicon-picture'></i> 
              </a>
              <a type="button" class="btn btn-warning btn-sm fun" data-toggle="modal" data-target="#editar<?php echo $row_usuario['id_jefes'];?>">
                <span class="glyphicon glyphicon-edit"></span>
              </a>
              <a class="btn btn-danger btn-sm fun" data-toggle="modal" data-target="#eliminar<?php echo $row_usuario['id_jefes']; ?>">
